...
export default class Encryption {
     ...
     */
    derivePassword(password: string, keySize = 32, iv?: string): EncryptionObject {
     ...
     * @param keySize
     */
    generateRandomKey(keySize: number): string {
        // random bytes
     ...
     * @param string
     * @param encryptionKey
     * @returns {EncryptionObject}
     */
    encryptString(string: string, encryptionKey: string): EncryptionObject {
     ...
     * @returns {string}
     */
    decryptString(encryptedString: string, key: string, iv: string): string {
    ...
    /**
     * Get single string containing value and IV
     * @param val
     * @param key
     * @returns {string}
     */
    getEncryptionString(val: string, key: string): string {
        const encrypted = this.encryptString(val, key);
        return encrypted.iv + '~' + encrypted.encryptedString;
    }

    /**
     * Get value from encrypted string with IV
     * @param val
     * @param key
     * @returns {string}
     */
    getEncryptionValue(val: string, key: string): string {
        return !val ? null : this.decryptString(val.split('~')[1], key, val.split('~')[0]);
    }
}
