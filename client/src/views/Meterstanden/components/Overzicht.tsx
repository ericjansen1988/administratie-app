...
const Overzicht = () => {
  ...
  const haalDataOp = async () => {
    ...
    try{
      const refreshedtoken = await refreshOauth(user, '/api/oauth/refresh/enelogic', userInfoRef.current.enelogic.token, updateEnelogicSettings(ref, userInfoRef.current.enelogic))
