...
export const checkPreconditions = (accounts: any, rekeningen: any, options: any = {}) => {
  //check
  ...
  const algemeen_account = accounts.find((account: any) => account.description === from_account);
  let currentstate: any = {...initialPreconditions};
...
export const runSalarisVerdelenScript = async (rekeningen: any, options: any = {}) => {
  const maandnummer = options.maandnummer || (new Date()).getMonth()+1;
...
export const deleteBunqSettings = async (ref: any) => {
