...
const getDifferenceArray = async (array: any, id: string, columnArray: any) => {
  ...
  for(var item of array){
    let index = array.findIndex((e: any) => e[id] === item[id]);
...
const addSolarEdgeData = async (data: any, start: string, end: string, timeframe: string, solarEdgeConfig: any, user: any) => {
  ...
  try{
    ...
    for(let item of data){
      item['opwekking'] = null;
      let solaredgeitem = solaredgedata.find((entry: any) => moment(entry.date).isSame(moment(item.datetime_verbruik)));
